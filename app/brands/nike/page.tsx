...
export default function Nike() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      const { data, error } = await supabase.from("products").select("*").eq("brand", "Nike");
      ...
      <main>
        ...
        <section>
          <div className="relative w-full h-80">
            <Image
              src="/heroes/nike-hero.jpg"
              alt="A white shoe with red capital letter 'N' on it"
              fill
              className="object-cover object-[center_53%]"
            />
            <div className="absolute inset-0 bg-black/40" />
            <h1 className="absolute left-14 bottom-14 text-6xl font-bold text-white">Nike</h1>
          </div>
        </section>
        ...
        <section className="mx-auto grid max-w-360 gap-6 px-4 py-8 sm:px-6 lg:grid-cols-[240px_1fr] lg:px-8">
          ...
          <div>
            <div className="mb-6 flex flex-col justify-between gap-4 rounded-2xl bg-white p-5 text-black shadow-sm sm:flex-row sm:items-center">
              <div>
                <h2 className="text-xl font-bold">Nike</h2>
