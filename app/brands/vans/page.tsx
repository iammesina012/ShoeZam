...
export default function Vans() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      const { data, error } = await supabase.from("products").select("*").eq("brand", "Vans");
      ...
      <main>
        ...
        <section>
          <div className="relative w-full h-80">
            <Image
              src="/heroes/vans-hero3.jpg"
              alt="A white shoe with red capital letter 'N' on it"
              fill
              className="object-cover object-[center_61%]"
            />
            <div className="absolute inset-0 bg-black/40" />
            <h1 className="absolute left-14 bottom-14 text-6xl font-bold text-white">Vans</h1>
          </div>
        </section>
