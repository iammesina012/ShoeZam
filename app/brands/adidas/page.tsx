...
export default function Adidas() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      const { data, error } = await supabase.from("products").select("*").eq("brand", "Adidas");
      ...
      <main>
        ...
        <section>
          <div className="relative w-full h-80">
            <Image
              src="/heroes/adidas-hero2.jpg"
              alt="A white floating shoe against a black brick wall background."
              fill
              className="object-cover object-[center_52%]"
            />
            <div className="absolute inset-0 bg-black/40" />
            <h1 className="absolute left-14 bottom-14 text-6xl font-bold text-white">Adidas</h1>
          </div>
        </section>
